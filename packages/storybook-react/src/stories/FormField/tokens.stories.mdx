<!-- @license CC0-1.0 -->

import { Meta } from "@storybook/addon-docs";
import tokensDefinition from "@utrecht/components/form-field/tokens.json";
import tokens from "@utrecht/design-tokens/dist/index.json";
import { ComponentTokensSection } from "@utrecht/documentation/components/ComponentTokensSection.jsx";

<Meta
  id="react-form-field--design-tokens"
  title="React.js Component/Form field/Design Tokens"
  parameters={{
    status: {
      type: "WORK IN PROGRESS",
    },
  }}
/>

# Form Field Design Tokens

<ComponentTokensSection tokens={tokens} definition={tokensDefinition} component="utrecht-form-field" />
