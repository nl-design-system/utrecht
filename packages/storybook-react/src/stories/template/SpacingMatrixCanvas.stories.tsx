...
import {
  ...
  StoryShell,
  useSpacingStory,
} from './spacing-story-kit-canvas.js';
import {
  Accordion,
  AccordionProvider,
  ActionGroup,
...
const meta = {
  title: 'Prototypes/Spacing Matrix Canvas/Testen',
  id: 'prototype-spacing-matrix-canvas',
  component: RichText,
  parameters: {
    spacingDebug: true,
    docs: {
      description: {
        component: `
De **Spacing Matrix Canvas** is een alternatieve implementatie van de spacing matrix testomgeving die een \`<canvas>\` overlay gebruikt in plaats van \`::after\` pseudo-elementen.

Het voordeel van deze aanpak is dat de canvas buiten de DOM-boom van de componenten staat en dus nooit conflicteert met bestaande \`::after\` styling of \`:has(+ ...)\` selectors die juist worden getest.
Bovendien kan de canvas versie alle componenten tracken, niet alleen degenen die \`::after\` ondersteunen.

## Gebruik

Klik op het raster-icoon in de toolbar om de debug-overlay te tonen of te verbergen.
De overlay visualiseert de daadwerkelijk berekende witruimte onder elk element als een gekleurde balk op de canvas.
