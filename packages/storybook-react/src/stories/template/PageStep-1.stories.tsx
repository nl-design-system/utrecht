...
export const One: StoryObj<typeof Page> = {
  render: (args) => {
    ...
    const onContactDetailsSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    const onContactDetailsChange = () => {
      if (emailRef.current && emailRef.current.value) {
        if (emailRef.current.getAttribute('aria-invalid') && emailRef.current.validity.valid) {
          emailRef.current.removeAttribute('aria-invalid');
          setInvalid(false);
        } else if (emailRef.current && emailRef.current.value === '' && !emailRef.current.validity.valid) {
          emailRef.current.setAttribute('aria-invalid', 'true');
          setInvalid(true);
        }
      }
        ...
        <PageContent role="main">
          ...
          <div className="utrecht-demo-col--8">
            ...
            <section>
              ...
              <form onSubmit={onContactDetailsSubmit} noValidate>
                <FormField>
                  <p>
                    <FormLabel htmlFor="telefoonnummer">Telefoonnummer</FormLabel>
                  </p>
                  <Textbox ref={phoneRef} id="telefoonnummer" />
                </FormField>
                <FormField invalid={invalid}>
                  ...
                  <Textbox
                    ...
                    type="email"
                  />
                  <FormFieldDescription invalid={invalid} id="email-desc">
                    {invalid ? 'dit veld is verplicht' : 'Vragen met een * zijn verplicht.'}
                  </FormFieldDescription>
                </FormField>
