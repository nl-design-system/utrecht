...
import { getContrast, setupBody, setupDialog, setupScenario1 } from './lib';

// Extend the TypeScript definition with CSS properties that are not part of any CSS specification yet.
declare global {
...
describe('Root component', () => {
  describe('without design tokens', () => {
    ...
    it('has sufficient contrast between background-color and color', async () => {
      ...
      expect(getContrast(color, backgroundColor)).toBeGreaterThanOrEqual(4.5);
    });
  });
  ...
  describe('hostile CSS that disables subpixel-antialised font smoothing', () => {
    beforeEach(() => {
      ...
      const css = (str: TemplateStringsArray): string => str.join('');
