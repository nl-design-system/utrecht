...
function Currency(currency) {
  ...
  this.toMoney = function (number, isNumeric) {
    ...
    var symbol_left = this.currency['symbol_left']
      ? this.currency['symbol_left'] + this.currency['symbol_padding']
      : '';
    var symbol_right = this.currency['symbol_right']
...
function gformCleanNumber(text, symbol_right, symbol_left, decimal_separator) {
  ...
  for (var i = 0; i < text.length; i++) {
    ...
    if ((parseInt(digit, 10) >= 0 && parseInt(digit, 10) <= 9) || digit == decimal_separator) clean_number += digit;
    else if (digit == '-') is_negative = true;
  }

  //Removing thousand separators but keeping decimal point
  for (var i = 0; i < clean_number.length; i++) {
    digit = clean_number.substr(i, 1);
    if (digit >= '0' && digit <= '9') float_number += digit;
...
function gformDeleteUploadedFile(formId, fieldId, deleteButton) {
  ...
  parent
    .find('input[type="file"],.validation_message,#extensions_message_' + formId + '_' + fieldId)
...
function gformGetShippingPrice(formId) {
  ...
  if (shippingField.length == 1 && !gformIsHidden(shippingField)) {
    if (shippingField.attr('readonly')) shipping = shippingField.val();
    else shipping = gformGetPrice(shippingField.val());
  }
...
function gformCalculateProductPrice(form_id, productFieldId) {
  ...
  jQuery('.gfield_option' + suffix + ', .gfield_shipping_' + form_id)
    .find('select')
    .each(function () {
...
function gformGetOptionLabel(element, selected_value, current_price, form_id, field_id) {
  ...
  var price_label =
    element[0].tagName.toLowerCase() == 'option' ? diff : "<span class='ginput_price'>" + diff + '</span>';
...
function gformGetProductIds(parent_class, element) {
  var classes = jQuery(element).hasClass(parent_class)
    ? jQuery(element).attr('class').split(' ')
    : jQuery(element)
...
function gformRegisterPriceField(item) {
  ...
  for (var i = 0; i < _gformPriceFields[item.formId].length; i++)
    if (_gformPriceFields[item.formId][i] == item.productFieldId) return;
...
function gformShowPasswordStrength(fieldId) {
  ...
  jQuery('#' + fieldId + '_strength_indicator')
  ...
  $cells.each(function () {
    ...
    if ($subContainer.length > 0) {
      resetContainerItems = function ($c) {
        $c.children('.gfield_repeater_items')
...
function gformMatchCard(id) {
  ...
  if (!cardType) {
    ...
    jQuery(cardContainer)
      .find('.gform_card_icon_' + cardType)
...
function gformToggleCreditCard() {
  if (jQuery('#gform_payment_method_creditcard').is(':checked')) jQuery('.gform_card_fields_container').slideDown();
  else jQuery('.gform_card_fields_container').slideUp();
}
...
function gformInitChosenFields(fieldList, noResultsText) {
  return jQuery(fieldList).each(function () {
    ...
    var isConvoForm =
      typeof gfcf_theme_config !== 'undefined'
        ? gfcf_theme_config !== null && typeof gfcf_theme_config.data !== 'undefined'
...
var GFMergeTag = function () {
  ...
  GFMergeTag.getMergeTagValue = function (formId, inputId, modifier) {
    ...
    var isCopyPreviousAddressChecked =
      jQuery('#input_' + formId + '_' + fieldId + '_copy_values_activated:checked').length > 0;
    ...
    var inputSelector =
      fieldId == inputId ? 'input[name^="input_' + fieldId + '"]' : 'input[name="input_' + inputId + '"]';
    var input = field.find(
      inputSelector + ', select[name^="input_' + inputId + '"], textarea[name="input_' + inputId + '"]',
    );
    ...
    if (input.length === 1) {
      if (
        (input.is('select') || input.prop('type') === 'radio' || input.prop('type') === 'checkbox') &&
        modifier === ''
      ) {
        if (input.is('select')) {
...
var GFCalc = function (formId, formulaFields) {
  ...
  this.runCalc = function (formulaField, formId) {
    ...
    if (formattedResult !== false) {
      ...
      result = gformFormatNumber(
        result,
        !gformIsNumber(formulaField.rounding) ? -1 : formulaField.rounding,
        decimalSeparator,
        thousandSeparator,
      );
    }
  ...
  this.bindCalcEvents = function (formulaField, formId) {
    ...
    for (var i in matches) {
      ...
      var fieldId = parseInt(inputId, 10);
      var input = jQuery('#field_' + formId + '_' + fieldId).find(
        'input[name="input_' + inputId + '"], select[name="input_' + inputId + '"]',
      );
...
(function (gfMultiFileUploader, $) {
  ...
  $(document).ready(function () {
    if (
      (typeof adminpage !== 'undefined' && adminpage === 'toplevel_page_gf_edit_forms') ||
  ...
  function setup(uploadElement) {
    ...
    gfMultiFileUploader.toggleDisabled = function (settings, disabled) {
      var button =
        typeof settings.browse_button == 'string' ? $('#' + settings.browse_button) : $(settings.browse_button);
    ...
    function addMessage(messagesID, message) {
      $('#' + messagesID).prepend(
        "<li class='gfield_description gfield_validation_message'>" + htmlEncode(message) + '</li>',
      );
      // Announce errors.
      setTimeout(function () {
    ...
    uploader.bind('FilesAdded', function (up, files) {
      ...
      $.each(files, function (i, file) {
         ...
         */
        statusMarkup = gform
          .applyFilters('gform_file_upload_status_markup', statusMarkup, file, size, strings, removeFileJs, up)
    ...
    uploader.bind('Error', function (up, err) {
      if (err.code === plupload.FILE_EXTENSION_ERROR) {
        var extensions =
          typeof up.settings.filters.mime_types != 'undefined'
            ? up.settings.filters.mime_types[0].extensions /* plupoad 2 */
            : up.settings.filters[0].extensions;
        addMessage(
          up.settings.gf_vars.message_id,
          err.file.name + ' - ' + strings.invalid_file_extension + ' ' + extensions,
        );
    ...
    uploader.bind('FileUploaded', function (up, file, result) {
      ...
      if (typeof gf_legacy !== 'undefined' && gf_legacy.is_legacy) {
        html =
          ...
          imagesUrl +
          "/delete.png' " +
          "onclick='gformDeleteUploadedFile(" +
          formId +
          ',' +
          fieldId +
          ", this);' " +
...
function gformInitSpinner(formId, spinnerUrl, isLegacy = true) {
  ...
  jQuery('#gform_' + formId).on('submit', function () {
    ...
    var $spinnerTarget = gform.applyFilters(
...
function gformInitializeSpinner(formId, target, uniqId = 'gform-ajax-spinner') {
  if (jQuery('#gform_ajax_spinner_' + formId).length == 0) {
    var loaderHTML =
      '<span data-js-spinner-id="' + uniqId + '" id="gform_ajax_spinner_' + formId + '" class="gform-loader"></span>';
...
function gformAddSpinner(formId, spinnerUrl) {
  ...
  if (jQuery('#gform_ajax_spinner_' + formId).length == 0) {
    ...
    $spinnerTarget.after(
      '<img id="gform_ajax_spinner_' + formId + '"  class="gform_ajax_spinner" src="' + spinnerUrl + '" alt="" />',
    );
  }
}
...
function gformReInitTinymceInstance(formId, fieldId) {
  ...
  if (!editor) {
    gform.console.error(
      'gformReInitTinymceInstance did not find an instance for input_' + formId + '_' + fieldId + '.',
    );
    return;
  }
