...
export const Template = ({ items }) => `<div class="utrecht-navhtml">
   <nav class="utrecht-sidenav">
    <ul class="utrecht-sidenav__list">${items
      .map(
        ({ href, title, current, focus, children, siblings }) => `<li class="${clsx(
          "utrecht-sidenav__item",
          siblings && "utrecht-sidenav__item--sibling"
        )}">
          <a class="${clsx(
            "utrecht-sidenav__link",
            current && "utrecht-sidenav__link--current",
            focus && "utrecht-sidenav__link--focus",
            siblings && "utrecht-sidenav__link--sibling"
          )}" href="${href}">${title}</a>
          <ul>${
            children
              ? children
  ...
  <Story
    ...
    args={{
      items: [
        ...
        { href: "#", title: "Sibling #1", siblings: true },
        { href: "#", title: "Sibling #2", siblings: true },
        { href: "#", title: "Sibling #3", siblings: true },
      ],
    }}
  >
    {Template.bind({})}
