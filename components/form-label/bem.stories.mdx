<!--
@license EUPL-1.2
Copyright (c) 2021 Robbert Broersma
-->

import { Meta, Story, Canvas, ArgsTable } from "@storybook/addon-docs";
import clsx from "clsx";

<!-- Import component and component styles -->

import "./bem.css";

<!-- Import Docs -->

import README from "./README.md";

export const Template = ({ checked, content, disabled, type }) =>
  `<span class="${clsx(
    "utrecht-form-label",
    type === "checkbox" && "utrecht-form-label--checkbox",
    checked && "utrecht-form-label--checked",
    disabled && "utrecht-form-label--disabled",
    type === "radio" && "utrecht-form-label--radio"
  )}">${content}</span>`;

<Meta
  title="Components/Form label"
  argTypes={{
    content: {
      description: "Set the content of the label",
      control: "text",
      defaultValue: "Username",
    },
    type: {
      description: "Set the type of the form control",
      options: ["default", "checkbox", "radio"],
      control: { type: "radio" },
    },
  }}
  parameters={{
    docs: {
      transformSource: (_src, { args }) => Template(args),
    },
    status: {
      type: "WORK IN PROGRESS",
    },
    notes: {
      UX: README,
    },
  }}
/>

# Label component

Styling via the `.utrecht-form-label` class name:

<Canvas>
  <Story name="Label">{Template.bind({})}</Story>
</Canvas>

<ArgsTable story="Label" />

# Label for disabled input

Styling via the `.utrecht-form-label--disabled` class name:

<Canvas>
  <Story name="Label for disabled input" args={{ disabled: true }}>
    {Template.bind({})}
  </Story>
</Canvas>

# Label for checkbox

Styling via the `.utrecht-form-label--checkbox` class name:

<Canvas>
  <Story name="Label for checkbox" args={{ type: "checkbox" }}>
    {Template.bind({})}
  </Story>
</Canvas>

# Label for checked checkbox

Styling via the `.utrecht-form-label--checked` class name:

<Canvas>
  <Story name="Label for checked checkbox" args={{ type: "checkbox", checked: true }}>
    {Template.bind({})}
  </Story>
</Canvas>

# Label for radio button

Styling via the `.utrecht-form-label--radio` class name:

<Canvas>
  <Story name="Label for radio button" args={{ type: "radio" }}>
    {Template.bind({})}
  </Story>
</Canvas>

# Label for checked radio button

Styling via the `.utrecht-form-label--radio` class name:

<Canvas>
  <Story name="Label for radio button" args={{ type: "radio", checked: true }}>
    {Template.bind({})}
  </Story>
</Canvas>
