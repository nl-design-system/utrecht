...
export const URLTemplateExample = ({ url = "", link = false }) => {
  let html = URLTemplate({ url });
  return link ? `<a class="utrecht-link" href="${url}">${html}</a>` : html;
};
...
<Canvas>
  <Story
    ...
    parameters={{
      docs: {
        transformSource: (_src, { args }) => URLTemplateExample(args),
      },
    }}
  >
    {URLTemplateExample.bind({})}
  </Story>
</Canvas>
